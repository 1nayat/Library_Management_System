...
namespace Library_Management_System.Repositories
{
    public class BookRepository : IBookRepository
    {
        ...
        {
            using var conn = _dbContext.CreateConnection();
            const string sql = @"INSERT INTO Book4 (BookName, Author, Detail, Price, Publication, Branch, Quantities, AvailableQnt, RentQnt, Image, BookPDF) 
                                 VALUES (@BookName, @Author, @Detail, @Price, @Publication, @Branch, @Quantities, @AvailableQnt, @RentQnt, @Image, @BookPDF)";
        ...
        public async Task UpdateAsync(Book book)
        {
            ...
            const string sql = @"UPDATE Book4 
                                 SET BookName=@BookName, Author=@Author, Detail=@Detail, Price=@Price, 
                                     ...
                                     AvailableQnt=@AvailableQnt, RentQnt=@RentQnt, Image=@Image, BookPDF=@BookPDF 
                                 WHERE BookID=@BookID";
            await conn.ExecuteAsync(sql, book);
        }
        ...
        public async Task<PaginatedList<Book>> GetPaginatedBooksAsync(string search = null, int page = 1, int pageSize = 10)
        {
            using var conn = _dbContext.CreateConnection();
